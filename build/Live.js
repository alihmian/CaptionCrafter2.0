...
// Create the Bot
// ----------------------
const bot = new grammy_1.Bot("8004701605:AAGkoRfckJZ-EJncDhDD5ZxdE9AWa4IuKks");
...
function getOutputPath(ctx) {
    return ctx.session?.outputPath
        ?? `./OutPut/live_image_${ctx.from.id}.png`;
}
...
async function updateNewspaperImage(ctx) {
    ...
    const pythonScript = "./src/craft/Live.py"; // Adjust if needed
    const args = [
        pythonScript,
...
async function clearFormConversation(conversation, ctx) {
    ...
    await ctx.editMessageMedia({
        type: "photo",
        media: new grammy_1.InputFile("./assets/LIVE_TEMPLATE.png"),
    });
...
bot.command("start", async (ctx) => {
    const userId = ctx.from?.id;
    const userSpecificPath = `./OutPut/live_image_${userId}.png`;
    log(`User ${userId} started the bot`);
    ctx.session.outputPath = userSpecificPath;
    const sentMessage = await ctx.replyWithPhoto(new grammy_1.InputFile("./assets/LIVE_TEMPLATE.png"), { reply_markup: exports.formMenu });
