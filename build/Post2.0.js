...
function getOutputPath(ctx) {
    return ctx.session?.outputPath
        ?? `./OutPut/generated_post2.0_image_${ctx.from.id}.png`;
}
...
async function updateNewspaperImage(ctx) {
    ...
    const pythonScript = "./src/craft/Post2.0.py"; // Adjust if needed
    const args = [
        pythonScript,
...
function buildFormMenu(conversation, data) {
    return conversation.menu("form")
        ...
        .row()
        // Event1 (was Text3)
        .text(data.Events ? "رویداد ✅ " : "رویداد ❌", (ctx) => ctx.conversation.enter("eventsConversation"))
        .row()
...
bot.command("start", async (ctx) => {
    const userId = ctx.from?.id;
    const userSpecificPath = `./OutPut/generated_post2.0_image_${userId}.png`;
