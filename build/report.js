...
// Create the Bot
// ----------------------
const bot = new grammy_1.Bot("8212516791:AAFXwlMxvbxRg0bEvh2J0sdQ2O8NaXoXdqM");
...
function getOutputPath(ctx) {
    return ctx.session?.outputPath
        ?? `./OutPut/generated_report_image_${ctx.from.id}.png`;
}
...
async function updateNewspaperImage(ctx) {
    ...
    const pythonScript = "./src/craft/report.py"; // Adjust if needed
    const args = [
        pythonScript,
...
async function clearFormConversation(conversation, ctx) {
    ...
    await ctx.editMessageMedia({
        type: "photo",
        media: new grammy_1.InputFile("./assets/report.png"),
    });
...
bot.command("start", async (ctx) => {
    const userId = ctx.from?.id;
    const userSpecificPath = `./OutPut/generated_report_image_${userId}.png`;
    log(`User ${userId} started the bot`);
    ctx.session.outputPath = userSpecificPath;
    const sentMessage = await ctx.replyWithPhoto(new grammy_1.InputFile("./assets/report.png"), { reply_markup: exports.formMenu });
