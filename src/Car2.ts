...
//  Create the Bot
// --------------------------------------------------
const bot = new Bot<MyContext>("8126184438:AAHHqLI2wV8cBuvUyi9iW8P4pGFk-HgjjEQ");
...
async function clearFormConversation(
    ...
    await ctx.editMessageMedia({
        type: "photo",
        media: new InputFile("./assets/CAR2_TEMPLATE.png"),
    });
    await ctx.editMessageReplyMarkup({ reply_markup: clearedMenu });
}
...
bot.command("start", async (ctx) => {
    ...
    const sentMsg = await ctx.replyWithPhoto(new InputFile("./assets/CAR2_TEMPLATE.png"), { reply_markup: formMenu });
